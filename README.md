...
- 5. A file named `SC2Switcher_x64.exe - Shortcut` should appear in the unzipped folder. Run it.
    - StarCraft II should load as normal, but the login screen will look a bit different, similar to `![this](replay_fix_1.png)`.
    - After a few seconds, the game will hang on a "Connecting to Blizzard services" dialog, as in `![this](replay_fix_2.png)`.
